...
SELECT * FROM `clase` WHERE nombre_clase like "%oga%";

-- 10) Muestra las clases que se imparten en la Sala Yoga
select nombre_clase from clase where id_sala = 3;

-- 11) Muestra los socios que no tienen ninguna clase reservada

select nombre from socio where id_socio not in (select id_socio from clase);
...
update socio set telefono = "1234321" where id_socio = 1;

-- 13) Cambia la capacidad de la Sala Yoga a 30 personas

update sala set capacidad_sala = 30 where nombre_sala = "Sala Yoga";

-- 14) Cambia el instructor de la clase con id 3

update clase set id_instructor = 1 where id_clase = 3;

-- 15) Cambia la sala de todas las clases de Spinning a la Sala B

UPDATE clase set id_sala = 1 where nombre_clase like "%pinning%";
...
-- Cannot delete or update a parent row: a foreign key constraint fails (`gimnasio`.`clase`, CONSTRAINT `clase_ibfk_3` FOREIGN KEY (`id_socio`) REFERENCES `socio` (`id_socio`))

-- 18) Borra correctamente un socio:
--     primero sus clases y después el socio

delete from clase where id_socio = 1;
delete from socio where id_socio = 1;
